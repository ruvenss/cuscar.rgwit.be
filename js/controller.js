...
function export_manifest(arr_rows, callback) {
	...
	if (arr_rows.length>0) {
		...
		carrier_manifest = "data:application/octet-stream," + encodeURIComponent(carrier_manifest);
		//dialog.close();
		if (manifest_rows_checked===i){
			newWindow = window.open(carrier_manifest, 'neuesDokument');
			callback(true);
		}
	} else {
		error_front_end(1);
...
function export_data(arr_rows) {
	...
	if (arr_rows.length>0) {
		...
		cuscar_manifest= "data:application/octet-stream," + encodeURIComponent(cuscar_manifest);
		newWindow = window.open(cuscar_manifest, 'neuesDokument');
